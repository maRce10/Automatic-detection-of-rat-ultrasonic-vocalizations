...
flowchart LR
  A(Format sound files) --> B(Detect sounds) 
  B --> C("Measure acoustic features")
  C --> D(Mitigate false positives)
  D --> E(Summarize USVs counts) 
...
# order columns
highes_min <- highes_min[, c("sound.files", "start", "end",  "start (min:s)", "end (min:s)", "duration", "counts", "rate")]
...
- For the sake of the example, lest's say we have a mate entry at different times for each sound file. Lets simulate that data:
```{r}

mate_entry <- data.frame(
    sound.files = unique(highes_min$sound.files),
    time = c(150, 300, 450)
)
